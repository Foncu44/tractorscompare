...
export default async function BrandsPage({
  ...
  const formAction = pathForLocale('brands', loc);
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="border-b border-gray-200 bg-white py-4">
        <div className="container-custom">
          <form className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between" action={formAction} method="GET">
        ...
        <div className="container-custom">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {brands.map(({ brand, slug, count, logo }) => (
              <Link
                key={brand}
                href={pathForLocale(`brands/${slug}`, loc)}
          ...
          {brands.length === 0 && (
            <div className="text-center py-12 text-gray-600">
              No brands found matching &quot;{resolvedParams.q}&quot;.
            </div>
          )}
        </div>
      </section>
        ...
        <div className="container-custom">
          <div className="max-w-4xl mx-auto prose prose-lg prose-sm md:prose-lg">
            ...
            <p className="text-base md:text-lg text-gray-700 mb-4 md:mb-6 leading-relaxed">
              TractorsCompare provides comprehensive tractor data for {allBrands.length} major tractor manufacturers, covering over {tractors.length.toLocaleString()} tractor models with detailed technical specifications.
            </p>
          </div>
        </div>
      </section>
