...
export default function CompararPage() {
  ...
  const specsToCompare = [
    { label: 'Brand', key: 'brand' as keyof Tractor },
    { label: 'Model', key: 'model' as keyof Tractor },
    { label: 'Year', key: 'year' as keyof Tractor },
    ...
    <div className="container-custom py-12">
              ...
              {filteredTractors.map((tractor) => (
                ...
                >
                  ...
                  {selectedTractors.some(t => t.id === tractor.id) && (
                    <span className="text-primary-600 text-sm">Already added</span>
                  )}
                </button>
              ))}
            </div>
      ...
      {selectedTractors.length > 0 ? (
                ...
                {selectedTractors.some(t => t.dimensions) && (
                  <>
                    ...
                    {['length', 'width', 'height'].map((dim) => (
                      <tr key={dim}>
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 sticky left-0 bg-white z-10">
                          {dim === 'length' ? 'Length' : dim === 'width' ? 'Width' : 'Height'} (mm)
                        </td>
                        {selectedTractors.map((tractor) => (
                          <td key={tractor.id} className="px-6 py-4 text-sm text-center text-gray-700">
                ...
                {selectedTractors.some(t => t.hydraulicSystem) && (
                  <>
                    ...
                    {['pumpFlow', 'liftCapacity'].map((hyd) => (
                      <tr key={hyd}>
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 sticky left-0 bg-white z-10">
                          {hyd === 'pumpFlow' ? 'Pump Flow (L/min)' : 'Lift Capacity (kg)'}
                        </td>
                        {selectedTractors.map((tractor) => (
                          <td key={tractor.id} className="px-6 py-4 text-sm text-center text-gray-700">
        ...
        <div className="text-center py-16 bg-white rounded-lg shadow-md">
          ...
          <h3 className="text-2xl font-bold mb-2">Start Comparing</h3>
          <p className="text-gray-600 mb-6">
            Add tractors to compare their specifications side by side.
          </p>
          <button
